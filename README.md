...
قبل المقارنة تُطبّق معالجة لغوية عربية متخصصة عبر مكتبة `arabic-stemmer`
(تجريد السوابق/اللواحق والتشكيل وتوحيد الهمزات) مع توحيد أولي
(`أإآ→ا` و`ة→ه` و`ى→ي`)، ثم تُقارَن كل الكلمات المرشحة للكلمة
(جذورها + صيغتها المطبّعة) مع مفاتيح القاموس — فيُكتشَف `والتداول`
و`الارباح` و`بالرابط` كما لو كانت `تداول` و`ارباح` و`رابط`.
الكلمات الإنجليزية تُقارَن بعد التحويل لأحرف صغيرة.
